...
upper_voltlim = simpledialog.askfloat(title="Voltage Range",
                                  prompt="Input upper voltage limit (add 0.1):")

voltage_range = np.arange(lower_voltlim, upper_voltlim, 0.2)
figsize = (8,6)
...
def Echem_Plots(filename):
    ...
    def voltage_profiles():
        ...
        def volt_vs_time():
            ...
            plt.title('Votlage vs. Cycle Time')
            plt.xlabel('Time (hours)')
            plt.ylabel('Votlage (V)')
            plt.yticks(voltage_range)
            plt.margins(0)
            return fig
        ...
        def ee():
            ...
            for cyclenumber, cycledata in data.items():
                ...
                plt.plot(v_cx, v_cy, color='red', alpha=agrad[cyclenumber])
                plt.plot(v_dx, v_dy, color='blue', alpha=agrad[cyclenumber])
            plt.title('Votlage vs. Electron Equivalence')
